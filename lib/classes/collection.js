...
function Collection(dbSettings) {
  ...
  this.model = null;
  this.orm = bookshelf(dbSettings);
}

const proto = {
  createSchema(schema) {
    if (!schema || !_.isObject(schema)) {
      throw new Error(`Collection Error: ${this.settings.tableName} schema must be an Object`);
    }

    const columnKeys = Object.keys(schema);

    return this.orm.knex.schema.createTableIfNotExists(this.settings.tableName, (table) => {
      let column;
  ...
  setModel(model) {
    ...
    const baseModel = Object.assign({}, defaults, model);

    this.model = this.orm.Model.extend.apply(this.orm, baseModel);
  },
